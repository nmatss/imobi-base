...
export function initializeScheduledJobs(): void {
  ...
  if (process.env.VERCEL) {
    console.log('[ScheduledJobs] Running on Vercel - skipping node-cron initialization');
    console.log('[ScheduledJobs] Cron jobs will be triggered via Vercel Cron HTTP endpoints');
    return;
  }
  ...
  try {
    ...
    console.warn('[ScheduledJobs] node-cron not available - skipping cron initialization');
    console.warn('[ScheduledJobs] If running on Vercel, this is expected. Jobs run via HTTP cron endpoints.');
    return;
  }

  console.log('[ScheduledJobs] Initializing node-cron scheduled jobs (persistent server mode)...');
  ...
  const jobs: Array<{
    ...
    { name: 'payment-reminders', schedule: '0 9 * * *', fn: runPaymentReminders, options: { timezone: 'America/Sao_Paulo' } },
    { name: 'daily-reports', schedule: '0 0 * * *', fn: runDailyReports, options: { timezone: 'America/Sao_Paulo' } },
    { name: 'weekly-reports', schedule: '0 8 * * 1', fn: runWeeklyReports, options: { timezone: 'America/Sao_Paulo' } },
    { name: 'monthly-reports', schedule: '0 8 1 * *', fn: runMonthlyReports, options: { timezone: 'America/Sao_Paulo' } },
    { name: 'cleanup-sessions', schedule: '0 * * * *', fn: runCleanupSessions },
    { name: 'cleanup-logs', schedule: '0 */6 * * *', fn: runCleanupLogs },
    { name: 'integration-sync', schedule: '0 */6 * * *', fn: runIntegrationSync },
    { name: 'database-backup', schedule: '0 2 * * *', fn: runDatabaseBackup, options: { timezone: 'America/Sao_Paulo' } },
    { name: 'cleanup-temp-files', schedule: '0 3 * * 0', fn: runCleanupTempFiles, options: { timezone: 'America/Sao_Paulo' } },
  ];
