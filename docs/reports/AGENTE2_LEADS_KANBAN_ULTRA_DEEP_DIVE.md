...
if (filters.tags.length > 0) {
  ...
  if (!filters.tags.some((tagId) => leadTagIds.has(tagId)))
    // Set.has() é O(1)
    return false;
}
```
...
app.post("/api/leads/bulk-update", async (req, res) => {
  ...
  try {
    ...
    const result = await db
      ...
      .where(
        and(inArray(leads.id, leadIds), eq(leads.tenantId, req.user.tenantId)),
      )
      .returning();
...
const fetchAllLeadTags = async () => {
  try {
    const res = await fetch("/api/leads/tags/batch", {
      credentials: "include",
    });
...
**SHOULD HAVE (P1):** 6. @dnd-kit para mobile 7. Parallel fetching 8. React.memo strategy 9. Error boundaries

**NICE TO HAVE (P2):** 10. Real-time WebSocket 11. Fuzzy search 12. Pull-to-refresh 13. Keyboard shortcuts 14. Analytics tracking
