...
.responsive-grid-5 {
  @apply grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5;
}
...
.kpi-card {
  @apply min-w-[160px] flex-shrink-0 sm:min-w-0;
}
...
<div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
  <div className="flex gap-2 sm:gap-3 min-w-max sm:grid sm:grid-cols-2 md:grid-cols-5">
    <Card className="min-w-[120px] sm:min-w-0">{/* KPI content */}</Card>
  </div>
</div>
```
...
<div className="sm:hidden space-y-3">
  {items.map((item) => (
    <Card key={item.id}>{/* Card content with all info */}</Card>
  ))}
</div>;
...
}
{
  mobileMenuOpen && (
    <div className="md:hidden border-t bg-background">
      <nav className="flex flex-col gap-3">{/* Navigation items */}</nav>
    </div>
  );
}
