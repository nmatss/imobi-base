...
```typescript
toast.withAction("Item excluído", "Desfazer", () => restoreItem(id));
```
...
const handleSubmit = async (data) => {
  ...
  await toast.promise(saveForm(data), {
    loading: "Salvando...",
...
const handleDelete = async (id: string) => {
  ...
  toast.withAction("Item excluído", "Desfazer", async () => {
...
const handleBulkDelete = async (ids: string[]) => {
  await toast.promise(Promise.all(ids.map((id) => deleteItem(id))), {
