...
const RENTAL_AI_PROMPTS: RentalAIPrompt[] = [
  ...
  },
  {
    ...
    template: (data) =>
      `O valor de ${data.oldValue} passara para ${data.newValue}...`,
  },
...
app.get("/api/rental-payments", async (req, res) => {
  ...
  const result = await storage.getRentalPaymentsByTenant(tenantId, {
    page,
    limit,
  });
...
const contractsWithPayments = await db
  .select()
  .from(contracts)
...
class NotificationService {
  async sendPaymentReminder(payment: Payment) {
    ...
    await sendSMS(
      renter.phone,
      `Lembrete: Aluguel de ${formatCurrency(payment.totalValue)} vence em ${formatDate(payment.dueDate)}`,
    );
  }
...
const { metrics, alerts, recentPayments, contracts } = await fetch(
