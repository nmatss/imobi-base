...
function ReportCard() {
  ...
  return (
    <Card>
      ...
      <CardContent>
        {loading ? <CardLoader lines={5} /> : <ReportContent data={report} />}
      </CardContent>
    </Card>
  );
}
...
function FinancialPage() {
  ...
  return (
    <div>
      ...
      {chartsLoading ? <FinancialChartsSkeleton /> : <Charts data={charts} />}
...
function PropertyList() {
  const { properties, hasMore, loadMore, isLoadingMore } =
  ...
  return (
    <div>
      ...
      {hasMore && (
        <div ref={loadMoreRef}>{/* Intersection observer trigger */}</div>
      )}
    </div>
  );
}
...
```tsx
<div role="status" aria-live="polite" aria-label="Carregando imóveis">
  <Skeleton />
...
```tsx
{
  loading ? <Skeleton className="h-64" /> : <div className="h-64">Content</div>;
}
```
