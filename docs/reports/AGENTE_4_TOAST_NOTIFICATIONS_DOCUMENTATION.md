...
```typescript
// Simple usage
toast.promise(saveProperty(data), {
...
toast.success(
  ...
  selectedLead
    ? "O lead foi atualizado com sucesso."
...
toast.success(
  ...
  "A interação foi adicionada ao histórico.",
);
```
...
const handleSave = async () => {
  ...
  await toast.promise(saveSettings(formData), {
    loading: "Salvando...",
...
```typescript
toast.withAction("Lead arquivado", "Desfazer", () => restoreLead(id));
```
