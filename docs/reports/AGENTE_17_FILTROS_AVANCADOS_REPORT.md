...
export default function PropertiesList() {
  ...
  const { filters, filteredProperties, updateFilters } =
    usePropertyFilters(properties);

  const cities = Array.from(new Set(properties.map((p) => p.city))).filter(
    Boolean,
  );
...
export default function LeadsKanban() {
  ...
  const sources = Array.from(new Set(leads.map((l) => l.source))).filter(
    Boolean,
  );
