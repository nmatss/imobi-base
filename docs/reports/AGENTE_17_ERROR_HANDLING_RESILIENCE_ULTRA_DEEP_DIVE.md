...
// Usage
app.post("/api/webhooks/stripe", idempotencyMiddleware, async (req, res) => {
  // Process webhook...
});
```
...
// Usage
app.get("/api/properties", bulkheadMiddleware("database"), async (req, res) => {
  // ...
});
```
...
// Usage
app.post("/api/ai/generate", featureGate("ai_features"), async (req, res) => {
  // AI endpoint
});
```
...
export const geocodeBreaker = new CircuitBreaker(async (address: string) => {
...
export function getCircuitBreakerHealth() {
  return {
    name: "GoogleMaps",
    state: geocodeBreaker.opened
      ? "OPEN"
      : geocodeBreaker.halfOpen
        ? "HALF_OPEN"
        : "CLOSED",
